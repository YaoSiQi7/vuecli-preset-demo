<template>
  <v-expansion-panels v-model="panel" class="my-0 py-0 elevation-3">
              ...
              <v-row class="ma-0 py-0">
                ...
                >
                  ...
                  <v-select
                    ...
                    no-data-text="没有匹配数据"
                    class="py-0"
                    @change="onHandleChange(items.value)"
                  >
    ...
    <v-dialog v-model="isdateShow" width="321">
      <v-date-picker selected-items-text="已选择日期" v-model="dates.dates" range  @change="dateChange"></v-date-picker>
    </v-dialog>
  </v-expansion-panels>
</template>
  ...
  methods: {
    ...
    dateChange () {
      ...
      if (dates.dates.length === 2) {
        if (moment(dates.dates[0]).valueOf() > moment(dates.dates[1]).valueOf()) {
          this.reverseArry(dates.dates);
        }
        for (let item of this.fieldList) {
          if (item.type === 'date' && (dates.placeholder === item.placeholder)) {
            item.value = dates.dates.join(' 至 ');
          }
        }
