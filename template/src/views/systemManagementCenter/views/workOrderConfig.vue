  ...
  <div>
    ...
    >
      ...
      <template v-slot:item.moduleName="{ item }">
        <span :title="item.moduleName">{{ item.moduleName | ellipsis(16) }}</span>
      </template>
              ...
              <v-stepper-items>
                ...
                <v-stepper-content step="1" class="elevation-0">
                  <v-card class="ma-2 elevation-0" style="height: 50vh">
                    ...
                    <v-divider></v-divider>
                    <v-form ref="configForm" v-model="valid" :lazy-validation="lazy">
...
export default {
  ...
  methods: {
    ...
    editGroup (item) {
      console.info('editGroup : ', item);
      let sessionStorage = window.sessionStorage;
      sessionStorage.setItem('moduleId', JSON.stringify(item.id));
      sessionStorage.setItem('currentFieldConfigTab', 1);
  ...
  computed: {
    ...
    searchForm () {
      ...
      if (this.searchParam[4].value) {
        formData['logType'] = this.searchParam[4].value;
      }
      return formData;
    },
    previewHeader () {
