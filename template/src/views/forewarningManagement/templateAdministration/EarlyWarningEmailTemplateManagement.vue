...
export default {
  data () {
    return {
      ...
      // 列表
      constHeader: [
        { text: '邮件模板名称', value: 'templateName', sortable: false },
        { text: '邮件模板内容', value: 'templateContent', sortable: false },
      ...
      editItemObj: '', // 编辑的当前项
      loading: false,
      searchFirstPlaceholder: '邮件模板名称',
      searchSecondPlaceholder: '邮件模板内容',
      ...
      tradeTypeProperty: [],
      // 增加按钮权限
      addTemplateBtn: '/addEmailTempButton', // 添加按钮是否展示
      editTemplateBtn: '/editEmailTempButton', // 当编辑按钮展示时，控制baseCreate中的编辑开关
      deleteTemplateBtn: '/deleteEmailTempButton' // 删除按钮权限
    };
  },
  components: {
    DelDialog,
  ...
  methods: {
    ...
    // 获取当前页的路由名称
    getPath () {
      localStorage.setItem('getMenu', '/earlyWarningEmailTemplateManagement');
    }
  }
};
</script>
