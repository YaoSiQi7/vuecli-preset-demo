<template>
  <div class="enu-content">
            ...
            >
              ...
              <template v-slot:body="{ items }">
                <tr v-for="(item, index) in items" :key="index" class="el-tr-defu">
                  ...
                  <td class="el-border-defu" :title="`${item.phone}`">{{ item.phone | ellipsis(11) }}</td>
    ...
    <base-create
      ...
      @handleChangeSelect="handleChangeSelect(arguments)"
...
export default {
  data () {
    return {
      ...
      filHeader: [
        ...
        { text: '姓名', value: 'userName', sortable: false, width: 100 },
      ...
      // 增加按钮权限
      addPowerBtn: '/addWarningPhoneMenuButton', // 控制添加目录按钮是否展示
      editPowerBtn: '/editWarningPhoneMenuButton', // 控制编辑目录按钮是否展示
      deletePowerBtn: '/deleteWarningPhoneMenuButton', // 控制删除目录按钮是否展示
      addGroupBtn: '/addWarningPhoneGroupInfoButton', // 添加组按钮是否展示
      baseCteateEditGroupBtn: '/editWarningPhoneGroupInfoButton', // 当编辑组按钮展示时，控制baseCreate中的编辑开关
      deleteGroupBtn: '/deleteWarningPhoneGroupInfoButton' // 删除组按钮权限
    };
  },
  watch: {
  ...
  methods: {
    ...
    // 添加和编辑组信息中的{姓名}change事件，联动效果是选择姓名之后，自动填充手机号
    handleChangeSelect (e) {
      for (let index = 0; index < this.nameAry.length; index++) {
        if (e[0].value === this.nameAry[index].id) {
          this.fieldFormData.forEach((im, ix, ay) => {
            if (im.label === '手机号*') {
              im.value = this.nameAry[index].phoneNumber;
            }
          });
        }
      };
    ...
    // 获取当前页的路由名称
    getPath () {
      localStorage.setItem('getMenu', '/earlyWarningPhoneGroupManagement');
    }
  },
  mounted () {
