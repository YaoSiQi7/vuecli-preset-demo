              ...
              <div v-if="addOrEditHeader.isEdit">
                <v-row>
                  ...
                  <v-col cols="7" justify-left v-show="isCanEdit" class="py-0">
                   <v-switch v-model="isEditMode" inset dense label="启用编辑" @change="onChange">
                   </v-switch>
                  </v-col>
                </v-row>
...
<script>
import { getList, addItem, editItem, deleteItem, searchNoQuoteLog } from '../../api/earlyWarningPolicyTemplateManagement.js';
...
export default {
  data () {
    return {
      ...
      // 列表
      constHeader: [
        { text: '预警短信模板名称', value: 'templateName', sortable: false },
        { text: '预警短信模板内容', value: 'templateContent', sortable: false },
      ...
      editItemObj: '', // 编辑的当前项
      loading: false,
      searchFirstPlaceholder: '预警短信模板名称',
      searchSecondPlaceholder: '预警短信模板内容',
  ...
  watch: {
    noteOrEmailTemplate: function (newVal, oldVal) {
      if (newVal === 0) {
        this.searchFirstPlaceholder = '预警短信模板名称';
        this.searchSecondPlaceholder = '预警短信模板内容';
        ...
        this.searchFirstPlaceholder = '预警邮件模板名称';
        this.searchSecondPlaceholder = '预警邮件模板内容';
        this.$refs.form.reset();
        this.constHeader = [
          { text: '预警邮件模板名称', value: 'templateName', sortable: false },
          { text: '预警邮件模板内容', value: 'templateContent', sortable: false },
          { text: '创建时间', value: 'createTime', sortable: false },
          { text: '操作', value: 'action', sortable: false, width: 100 }
        ];
        ...
        .then(res => {
          if (res.data.code === 200) {
            if (res.data.data.list.length > 0) {
              res.data.data.list.forEach((item, index, ary) => {
                ary[index].text = item.templateName;
                ary[index].value = item.id;
              });
              this.noteOrEmailTemplateName = res.data.data.list;
              this.flg = 2;
