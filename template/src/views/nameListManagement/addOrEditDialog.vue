              ...
              <div v-if="formHeader.isEdit">
                <v-row>
                  ...
                  <v-col cols="7" justify-left v-show="isCanEdit" class="py-0">
                   <v-switch v-model="isEditMode" inset dense label="启用编辑" @change="switchChange" :disabled='!editAndDelFlag'>
                   </v-switch>
                  </v-col>
                </v-row>
                <v-form
                  ...
                  <v-row
                    ...
                    <v-col cols="3" class="pr-0" >
                      <div class="mt-2" v-if ="item.isAutoShow">{{item.label}}:</div>
                    </v-col>
                    <v-col
                    ...
                    >
                      ...
                      />
                         ...
                         <v-col
                      v-if="item.type === 'select' && item.isAutoShow"
                      cols="7"
                  ...
                  >
                    ...
                    >
                      <v-select
                        ...
                        :multiple="item.multiple"
                        :attach="item.attach"
                        :fielType ="item.fielType"
                        outlined
                        dense
                        no-data-text="没有匹配数据"
                        @input="handleChangeSelect(item,index)"
                      />
                      <span v-if="item.comment" class="questionMark">
                        <v-tooltip right >
                          <template v-slot:activator="{ on }">
                    ...
                    <v-col
                      v-if="item.type === 'treeSelect' && item.isAutoShow"
                      cols="8"
                      class="mb-6"
                    >
...
export default {
  ...
  methods: {
    ...
    switchChange (fl) {
      console.log(fl, this.editInfoCode, '编辑开关是否可以用');
      this.$emit('switchOnChange', fl, this.editInfoCode);
    ...
    handleChangeSelect (item, index) {
      // console.log(this.isTreeVoild,'dfdf')
      //  if(!this.isTreeVoild){
      //    this.valid =true;
      //  }else{
      //     this.valid =false;
      //  }
      this.valid = this.isTreeVoild;
      // this.$refs.saveBtn
      // console.log(this.valid, this.$refs.saveBtn.disabled, ' this.hhhhvalid');
      this.$emit('handleChangeSelect', item, this.formData, index);
    },
    // 树形结构上
    normalizer (node) {
